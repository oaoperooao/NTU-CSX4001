...
reddit <- as.character(readtext("./corpus/reddit.txt"))
twitter <- as.character(readtext("./corpus/twitter.txt"))
tumblr <- as.character(readtext("./corpus/tumblr.txt"))
```

## 定義標題
```{r}
titles <- c("Reddit", "Twitter","Tumblr")

# store the content of the website
websites_content <- list(reddit,twitter,tumblr)
```
...
可見即使是相同的隱私權政策，三個網站的注重點仍然不同。reddit和大體相同，走平穩路線；twitter則是比較認真，將許多information製成data(或是只是想代換詞句？)；tumblr的隱私權，最主要和帳戶相關資料講得比較詳細。
```{r}
